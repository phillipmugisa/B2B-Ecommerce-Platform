...
class MembershipsDetailsView(AuthedOnlyAccessMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        try:
            braintree_client_token = braintree.ClientToken.generate({ "customer_id": self.request.user.id })
        except:
        ...
        if response.result.status in ("APPROVED", "CREATED"):
            if request.user.account_type == "SUPPLIER":
                membership = PaymentModels.Membership(
                    ...
                    plan = PaymentModels.MembershipPlan.objects.filter(id=plan_id).first(),
                    duration = "Monthly",
                )
                membership.save()
                ...
                if membership:
                    receipt = PaymentModels.MembershipReceipt(
                            ...
                            model_of_payment = PaymentModels.ModeOfPayment.objects.filter(name="Paypal").first(),

                            address=response.result.purchase_units[0].shipping.address.address_line_1,

                            payment_id = orderID,

                            amount_paid = float(response.result.purchase_units[0].amount.value),
        ...
        if result.status in ("submitted_for_settlement"):
            if request.user.account_type == "SUPPLIER":
                ...
                if plan:
                    ...
                    return redirect(reverse("payments:memberships-details", kwargs={"slug":plan.slug}))
                ...
                if membership:
                    receipt = PaymentModels.MembershipReceipt(
                            membership = membership,
                            model_of_payment = PaymentModels.ModeOfPayment.objects.filter(name="Braintree").first(),
